...
def build_product_lookup(max_items, skip_product_names):
    ...
    for row in products.itertuples(index=False):
        product_meta[int(row.product_id)] = {
    ...
    for path in order_product_paths:
        for chunk in pd.read_csv(path, usecols=["product_id"], chunksize=1_000_000):
            product_counter.update(int(value) for value in chunk["product_id"].values)
